...
// This selects the button inside the delete modal
const deleteBtn = document.querySelector("#deleteModal .btn-danger");

// assigning variables to the modals
const deleteModal = document.querySelector("#deleteModal");
const updateModal = document.querySelector("#createModal");

//getting the form for the addCardModal
addForm.addEventListener("submit", handleAddSubmit);
// window represents the page as a whole, so with this page is loaded the cards are displayed
window.addEventListener("load", displayAllCards);
// delete card on click
deleteBtn.addEventListener("click", deleteCard);

// show.bs.modal is bootstraps ability add an listener on the instance of a modal opening
...
function handleAddSubmit(event) {
  // submit automatically refreshes so thios prevents that. Used before saving cards may not need
  event.preventDefault();

  //putting the values entered by user into variables
  ...
  const idToUpdate = Number(
    event.target.closest("#createModal").dataset.cardId
  );
...
function setModalAttribute(event) {
  ...
  if (event.relatedTarget === addBtn) {
    ...
    //data-card-id
    event.target.dataset.cardId = cardTriggerModal;
...
function deleteCard(event) {
  ...
  const idToDelete = Number(
    event.target.closest("#deleteModal").dataset.cardId
  );
